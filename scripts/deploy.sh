...
#
# Passos: dependências -> build SSR -> (re)start no pm2.
# O react-router-serve escuta só em 127.0.0.1 (NODE_ENV=production); o Caddy expõe com SSL.
set -euo pipefail
...
# Precisa das devDependencies (vite, @react-router/dev) para o build, então NÃO usamos --production.
bun install --frozen-lockfile

echo "==> Build de produção (react-router build -> ./build)"
export NODE_ENV=production
# Gera ./build/client (estáticos) e ./build/server (handler SSR), consumidos
# pelo react-router-serve. O VITE_API_BASE_URL é embutido aqui a partir do .env.production.
bun run build

echo "==> (Re)start no pm2"
# O pm2 é uma app Node: o binário dele tem shebang `#!/usr/bin/env node`, então
# precisa de um `node` no PATH para rodar — independentemente de o NOSSO app rodar
# sob o bun (interpreter:"none" + `bun run start`). Sem node, o pm2 morre com
# "/usr/bin/env: 'node'..." e exit 127. Checamos antes para falhar com uma
# mensagem acionável em vez do erro críptico.
...
# startOrReload: sobe na primeira vez; nas próximas reinicia pegando o novo build.
